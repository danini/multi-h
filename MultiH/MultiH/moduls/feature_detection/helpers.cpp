...
void photometricallyNormalize(cv::Mat &image, const cv::Mat &binaryMask, float &sum, float &var)
{   
...
{
   cv::Mat ret(input.rows, input.cols, input.type());
...
{
   cv::Mat n(input.rows*2, input.cols*2, input.type());
