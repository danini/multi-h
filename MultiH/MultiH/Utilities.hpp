...
typedef std::vector<cv::Point_<BASE_T>> PointVec;
...
/*
inline 
void ReadVector(FILE *fptr, std::vector<cv::Point_<float>>& vec) {
	int vec_size;
	FscanfOrDie(fptr, "%d", &vec_size);
	vec.resize(vec_size);
	for (int i = 0; i<vec_size; ++i) {
...
inline 
void ReadVector(FILE *fptr, std::vector<cv::Point_<double>>& vec) {
	int vec_size;
	FscanfOrDie(fptr, "%d", &vec_size);
	vec.resize(vec_size);
	for (int i = 0; i<vec_size; ++i) {
...
inline 
void WriteVector(FILE *fptr, const std::vector<cv::Point_<BASE_T>>& vec) {
	if(vec.size() == 0) {
		fprintf(fptr, "%d\n", (int)vec.size());
	} else {
		fprintf(fptr, "%d ", (int)vec.size());
...
void ReadMatrix(FILE *fptr, cv::Mat_<float>& mat) {
	...
	mat = cv::Mat(dimRows,dimCols, cv::DataType<float>::type, cvcv::Scalar(0.));
...
void ReadMatrix(FILE *fptr, cv::Mat_<double>& mat) {
	...
	mat = cv::Mat(dimRows,dimCols, cv::DataType<double>::type, cv::Scalar(0.));
	for (int i = 0; i<dimRows; ++i)
		for (int j = 0; j<dimCols; ++j) {
			double val;
...
inline cv::Vec2d rectmin(const cv::Vec2d &a, const cv::Vec2d &b) {
	return cv::Vec2d(std::min(a[0], b[0]), std::min(a[1], b[1]));
}

inline cv::Vec2d rectmax(const cv::Vec2d &a, const cv::Vec2d &b) {
	return cv::Vec2d(std::max(a[0], b[0]), std::max(a[1], b[1]));
}

inline cv::Vec2d vecabs(const cv::Vec2d &a) {
	return cv::Vec2d(std::abs(a[0]), std::abs(a[1]));
}
...
/*inline
	int recoverPose( cv::InputArray E, const std::vector<cv::Point2d> &_points1, const std::vector<cv::Point2d> &_points2, cv::OutputArray _R,
	cv::OutputArray _t, cv::Mat K1, cv::Mat K2, cv::OutputArray _mask = noArray())
{
	cv::Mat K1i = K1.inv();
	cv::Mat K2i = K2.inv();
	cv::Mat apoints1 = cv::Mat(_points1);
	cv::Mat apoints2 = cv::Mat(_points2);
	...
	points1 = points1.t();
	points2 = points2.t();
	cv::Mat mucika = cv::Mat::ones(1, npoints, CV_64F);
	...
	cv::Mat R1, R2, t;
	decomposeEssentialcv::Mat(E, R1, R2, t);
...
namespace cv
{
	...
	public:
		...
		int run(cv::InputOutputArray _param0) const
		{
			cv::Mat param0 = _param0.getMat(), x, xd, r, rd, J, A, Ap, v, temp_d, d;
