...
#include "moduls/mode_seeking/MeanShiftClustering.h"
#include <chrono>
...
void MultiH::MergingStep(bool &changed)
{
	...
	cv::Mat featureVectors(N, 6, CV_64F);
	double *fv_ptr = (double *)featureVectors.data;
...
#endif
	{
		...
		*(fv_ptr + idx) = static_cast<double>(x1);
		*(fv_ptr + idx + 1) = static_cast<double>(y1);
		*(fv_ptr + idx + 2) = static_cast<double>(x2);
		*(fv_ptr + idx + 3) = static_cast<double>(y2);
		*(fv_ptr + idx + 4) = static_cast<double>(x3);
		*(fv_ptr + idx + 5) = static_cast<double>(y3);
	...
	MeanShiftClustering<double> mean_shift;
	mean_shift.Cluster(featureVectors, threshold_homography, resulting_clusters, cluster_points);
	...
	double *resulting_clusters_ptr = (double *)resulting_clusters.data;
...
void MultiH::EstablishStablePointSets()
{
	...
	cv::Mat featureVectors(N, 10, CV_64F);
	double *fv_ptr = (double *)featureVectors.data;
...
#endif
	{
		...
		*(fv_ptr + idx) = static_cast<double>(x1);
		*(fv_ptr + idx + 1) = static_cast<double>(x2);
		*(fv_ptr + idx + 2) = static_cast<double>(x3);
		*(fv_ptr + idx + 3) = static_cast<double>(y1);
		*(fv_ptr + idx + 4) = static_cast<double>(y2);
		*(fv_ptr + idx + 5) = static_cast<double>(y3);
		*(fv_ptr + idx + 6) = static_cast<double>(src_points[i].x * locality_lambda);
		*(fv_ptr + idx + 7) = static_cast<double>(src_points[i].y * locality_lambda);
		*(fv_ptr + idx + 8) = static_cast<double>(dst_points[i].x * locality_lambda);
		*(fv_ptr + idx + 9) = static_cast<double>(dst_points[i].y * locality_lambda);
	...
	vector<vector<int>> cluster_points;
	MeanShiftClustering<double> mean_shift;
	mean_shift.Cluster(featureVectors, threshold_homography, clusters, cluster_points);
	...
	for (int i = 0; i < clusters.rows; ++i)
	{
		...
		for (int j = 0; j < Ni; ++j)
		{
			const int idx = cluster_points[i][j];
