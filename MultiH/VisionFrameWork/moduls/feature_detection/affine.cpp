...
bool AffineShape::findAffineShape(const cv::Mat &blur, float x, float y, float s, float pixelDistance, int type, float response)
{
...
bool AffineShape::normalizeAffine(const cv::Mat &img, float x, float y, float s, float a11, float a12, float a21, float a22)
{
   ...
   { 
      ...
      cv::Mat smoothed(patchImageSize, patchImageSize, CV_32FC1, (void *)&workspace.front());
